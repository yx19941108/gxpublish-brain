  ...
  <div class="p-2 manuscript-review-task-page">
    <el-card shadow="hover">
      ...
      <el-table v-loading="loading" border :data="rows" row-key="id">
        ...
        <el-table-column label="任务状态" align="center" min-width="120">
          <template #default="scope">
            <dict-tag :options="wf_task_status" :value="scope.row.flowTaskStatus" />
          </template>
        </el-table-column>
        <el-table-column label="办理时间" align="center" prop="updateTime" min-width="180" />
        ...
        <template #empty>
          <el-empty description="暂无三审三校已办数据" />
        </template>
      </el-table>
...
import { buildTaskFinishViewRoute, MANUSCRIPT_REVIEW_TASK_FLOW_CODE } from './task-center';
...
const fetchList = async () => {
  loading.value = true;
  try {
    const resp = await pageByTaskFinish({ ...queryParams });
...
const handleView = (row: FlowTaskVO) => {
  void router.push(buildTaskFinishViewRoute(String(row.businessId), row.id));
};
