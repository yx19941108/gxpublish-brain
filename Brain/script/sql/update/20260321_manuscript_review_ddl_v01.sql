...
CREATE TABLE `brain_manuscript_review` (
  ...
  `manuscript_code` varchar(30) COLLATE utf8mb4_bin NOT NULL COMMENT '系统稿件号（SH|JD + yyyyMMdd + 三位流水号）',
  `external_manuscript_code` varchar(100) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '外部稿件编号（在审校模块内按租户唯一，允许为空）',

  `title` varchar(200) COLLATE utf8mb4_bin NOT NULL COMMENT '标题',
  `content` text COLLATE utf8mb4_bin NOT NULL COMMENT '正文（最大20000，校验由应用层控制）',
  `note` varchar(1000) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '说明（最大1000）',
  `media_channel_dict_code` bigint DEFAULT NULL COMMENT '媒体/栏目预设字典主键（sys_dict_data.dict_code；手填为空）',
  `media_channel_label` varchar(100) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '所属媒体/栏目展示快照（字典选择时冗余 dict_label；手填直接存文本；手填不回写预设）',
  `submitter_dept_id` bigint DEFAULT NULL COMMENT '报送部门ID',
  `submitter_dept_name` varchar(100) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '报送部门名称（展示用）',
  `author_names` varchar(200) COLLATE utf8mb4_bin DEFAULT NULL COMMENT '作者（仅中文顿号、分隔）',
